...
module OmniAuth
  module Strategies
    class Twitter < OmniAuth::Strategies::OAuth
      ...
      # DEPRICATED
      def friendships
        friendshipids = MultiJson.load(access_token.get('/1/friends/ids.json').body)
        MultiJson.load(access_token.get('/1/users/lookup.json?user_id=' + friendshipids['ids'][1..99].to_json).body)
      end
     
      # DEPRECATED
      def followers
        followerids = MultiJson.load(access_token.get('/1/followers/ids.json').body)
        MultiJson.load(access_token.get('/1/users/lookup.json?user_id=' + followerids['ids'][1..99].to_json).body)
      end
